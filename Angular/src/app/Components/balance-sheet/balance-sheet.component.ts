...
import { Assets } from 'src/app/Model/assets.model';
...
export class BalanceSheetComponent implements OnInit {
  ...
  total:Income[]=[];
  loan:Income[]=[];
  constructor(private http: HttpClient,private router: Router,private loginService: LoginService, private route: ActivatedRoute,private scenarioService:ScenarioService,private balanceSheetService: BalanceSheetService)
  {
  ...
  ngOnInit(): void {
    ...
    this.balanceSheetService.getBalanceSheetDetails(this.orgId,this.scenarioId).subscribe(response =>{
  ...
  onChange(event:any,col:any)
  {
    ...
    if(col == 'One')
    {
      ...
      this.balanceSheet.forEach(x=>{
        if(x.year == this.years[yearNumber].year && x.type=="Equipment")
        {
          this.equipment.push(x);
        }
        if(x.year == this.years[yearNumber].year && x.type=="Building")
        {
          this.building.push(x);
        }
        if(x.year == this.years[yearNumber].year && x.type=="Electronics")
        {
          this.electronics.push(x);
        }
        if(x.year == this.years[yearNumber].year && x.type=="Others")
        {
          this.other.push(x);
        }
        if(x.year == this.years[yearNumber].year && x.type=="Total")
        {
          this.total.push(x);
        }
        if(x.year == this.years[yearNumber].year && x.type=="Loan")
        {
          this.loan.push(x);
        }
      })
